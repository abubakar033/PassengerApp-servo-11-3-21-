...
//            selectBox.setHint(posData.geteInputType() + " " + posData.getvFieldName());
            ...
            multiLineText.setImeOptions(EditorInfo.IME_ACTION_DONE);
            multiLineText.setGravity(Gravity.CENTER | Gravity.START);
            multiLineText.setFloatingLabel(MaterialEditText.FLOATING_LABEL_NONE);
            multiLineTextArea.setVisibility(View.VISIBLE);
            multiLineText.setBothText(posData.getvFieldName(), posData.gettDesc());
    ...
    public void buildPackageTypes(MaterialEditText selectBox, String message, int viewId) {
        ...
        if (arr_data != null) {
            ...
            androidx.appcompat.app.AlertDialog.Builder builder = new androidx.appcompat.app.AlertDialog.Builder(getActContext());
            builder.setTitle(elementArrayList.get(viewId).getvFieldName());
            ...
            builder.setItems(cs_currency_txt, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int item) {
                    ...
                    String selectedVal = "" + list_packageType_items.get(item)[1];
                    elementArrayList.get(viewId).setSelectedOptionID(list_packageType_items.get(item)[0]);
