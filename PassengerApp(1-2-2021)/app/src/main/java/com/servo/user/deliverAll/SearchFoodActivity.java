...
public class SearchFoodActivity extends AppCompatActivity implements FoodSearchAdapter.OnItemClickList, AddFoodSearchItem {
    ...
    public void initView() {
        ...
        norecordTxt = (TextView) findViewById(R.id.norecordTxt);
        ...
        new CreateRoundedView(Color.parseColor("#ECECEC"), 15, 0,
                getActContext().getResources().getColor(android.R.color.transparent), searchTxtView);

        foodSearchRecycView = findViewById(R.id.foodSearchRecycView);
        foodSearchAdapter = new FoodSearchAdapter(getActContext(), foodArrList,this);
    ...
    @Override
    public void addSearchItem(HashMap<String, String> mapData) {
...
/*
        Bundle bn1= getIntent().getBundleExtra("data");

        bn.get("iCompanyId");
        bn.get("vCompany");
        bn.get("fMinOrderValue");
        bn.get("iMaxItemQty");
        bn.get("Restaurant_Status");
        bn.get("ispriceshow");



        map.put("iCompanyId", getIntent().getStringExtra("iCompanyId"));
        map.put("vCompany", getIntent().getStringExtra("vCompany"));
        map.put("fMinOrderValue", getIntent().getStringExtra("fMinOrderValue"));
        map.put("iMaxItemQty", getIntent().getStringExtra("iMaxItemQty"));
        ...
        HashMap<String ,String> paymentTypeHashmap= (HashMap<String, String>) getIntent().getSerializableExtra("paymentMethod");
    ...
    private void filterArrey(String seachKeyWord){
        ...
        foodArrList = new ArrayList<>();
        for (HashMap<String,String> e:arl){
            if (e.get("vItemType").trim().contains(seachKeyWord)){
                foodArrList.add(e);
            }
        }

        if (foodArrList.size()!=0){
        foodSearchAdapter = new FoodSearchAdapter(getActContext(), foodArrList,this);
        foodSearchRecycView.setLayoutManager(new LinearLayoutManager(getActContext()));
        foodSearchRecycView.setAdapter(foodSearchAdapter);
                    ...
                    if (isDataAvail == true) {
                        ...
                        if (restaurant_Arr != null) {
                            ...
                            for (int i = 0; i < restaurant_Arr.length(); i++) {
                              ...
                              //  if ( !generalFunc.getJsonValueStr("vImage", item_obj).equalsIgnoreCase("")){
