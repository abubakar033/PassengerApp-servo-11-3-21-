...
public class TrackOrderAdapter extends RecyclerView.Adapter<TrackOrderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (mapData.get("dDate").trim().equalsIgnoreCase("")) {
            ...
            holder.amPmTxtView.setVisibility(View.INVISIBLE);
        ...
        if (iStatusCode.equalsIgnoreCase("1")) {
            ...
            if (TrackOrderActivity.serviceId.equalsIgnoreCase("") || TrackOrderActivity.serviceId.equalsIgnoreCase("1")) {
                holder.statusImgView.setImageResource(R.drawable.track_status_order_pick);
            } else {
        ...
        if (mapData.get("eCompleted").equalsIgnoreCase("Yes")) {
            ...
            new CreateRoundedView(mContext.getResources().getColor(R.color.appThemeColor_1), Utils.dipToPixels(mContext, mapData.get("OrderCurrentStatusCode").equalsIgnoreCase(iStatusCode) ? 22 : 18), 0, mContext.getResources().getColor(R.color.appThemeColor_1), holder.imgContainerView);
        } else {
            int color=Color.parseColor("#dddddd");

            holder.titleTxtView.setTextColor(Color.parseColor("#000000"));
            holder.topView.setBackgroundColor(color);
            holder.statusImgView.setColorFilter(Color.parseColor("#5a5a5a"), PorterDuff.Mode.SRC_IN);
            new CreateRoundedView(Color.parseColor("#f8f8f8"), Utils.dipToPixels(mContext, mapData.get("OrderCurrentStatusCode").equalsIgnoreCase(iStatusCode) ? 22 : 18), Utils.dipToPixels(mContext, 2), color, holder.imgContainerView);
        }
        ...
        if (holder.bottomView.getVisibility() == View.VISIBLE && (nwPos) < listData.size()) {
            ...
            if (nextMapData.get("eCompleted").equalsIgnoreCase("Yes")) {
                ...
                holder.bottomView.setBackgroundColor(Color.parseColor("#dddddd"));
            }
        }
        ...
        if (mapData.get("OrderCurrentStatusCode").equalsIgnoreCase(iStatusCode)) {
            ...
            int size=Utils.dipToPixels(mContext, 44);
            params.height = size;
            params.width = size;
            ...
            LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) holder.imgContainerView.getLayoutParams();
            int size=Utils.dipToPixels(mContext, 36);
            params.height = size;
            params.width = size;
            holder.imgContainerView.setLayoutParams(params);

            int padding=Utils.dipToPixels(mContext, 4);
            holder.timeLineContainer.setPaddingRelative(padding, 0, padding, 0);
        }
    }
...
//        public TimelineView mTimelineView;
        ...
        public ViewHolder(View view) {
            ...
            contentTxtView = (TextView) view.findViewById(R.id.contentTxtView);
            ...
            amPmTxtView = (TextView) view.findViewById(R.id.amPmTxtView);
            titleTxtView = (TextView) view.findViewById(R.id.titleTxtView);
