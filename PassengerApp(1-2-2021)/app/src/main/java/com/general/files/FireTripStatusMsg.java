...
import com.servo.user.ChatActivity;
import com.servo.user.ConfirmEmergencyTapActivity;
import com.servo.user.HistoryDetailActivity;
...
public class FireTripStatusMsg {
    ...
    public void fireTripMsg(String message) {
        ...
        if (!GeneralFunctions.isJsonObj(finalMsg)) {
            ...
            if (passMessage.contains("@title@")) {
                //by umar
                String currentString = passMessage;
                String[] separated = currentString.split("@title@");
            String title = separated[0];
            String msgnew = separated[1];
            generalFunc.showGeneralMessage(title, msgnew);
        }else {
                ...
                if (eType.equalsIgnoreCase(Utils.CabGeneralType_UberX)) {
                    ...
                    if (MyApp.getInstance().getCurrentAct() instanceof OnGoingTripDetailsActivity && MyApp.getInstance().onGoingTripDetailsAct != null && MyApp.getInstance().onGoingTripDetailsAct.tripDetail.get("iTripId").equals(iTripId)) {
                        ...
                        if (MyApp.getInstance().getCurrentAct() instanceof OnGoingTripsActivity) {
                            ((OnGoingTripsActivity) MyApp.getInstance().getCurrentAct()).getOngoingUserTrips();
                        }
...
//                                    generalFunc.showGeneralMessage("", vTitle);
                                    if (vTitle.contains("@title@")) {
                                        //by umar
                                        String currentString = vTitle;
                                        String[] separated = currentString.split("@title@");
                                        String title = separated[0];
                                        String msgnew = separated[1];
                                        generalFunc.showGeneralMessage(title, msgnew);
